...
class NanoBananaService:
    ...
    async def _refine_logo(self, generated_image_path: str, reference_image_paths: list[str]) -> str | None:
        ...
        parts.append({"text": (
            ...
            "\nCRITICAL RULES FOR TEXT REPAIR: "
            "1. PRIORITY #1: The LOGO (e.g. 'SMMASH'). It is currently distorted. FIX IT. "
            "2. Treat the logo as a scalable vector graphic. Trace the EXACT curves of the letters from the reference. "
            "3. DO NOT output a blurry blob. The text must be CRISP and LEGIBLE, even if small. "
            "4. DO NOT 'correct' the spelling. If reference says 'SMMASH', do not write 'SMASH'. "
            "5. ERASE the distorted version and PAINT the correct version in its place. "
            "6. If the logo is wrapped around a curve (e.g. waist), warp the text correctly but keep the FONT identical. "
            "\nGENERAL RULES: "
            ...
            "- EQUAL IMPORTANCE to small logs and big logos. "
        ...
        for attempt in range(max_retries):
            try:
                ...
                prompt = (
                    ...
                    "same letters, same font, same size, same position, same color. "
                    "CRITICAL TEXT RULES: "
                    "0. LOGO AND TEXT MUST BE IDENTICAL. TREAT TEXT AS 'PROTECTED TRADEMARK GRAPHICS'. COPY EXACTLY. "
                    "1. DO NOT 'read' the text. Treat any text on the garment as abstract geometric SHAPES and SYMBOLS. "
                    "2. Trace these shapes pixel-for-pixel. Do not try to spell-check, correct, or re-type them. "
                    "3. If the text looks like 'SMMASH' or any other brand, copy the exact curves and lines of the letters. "
                    "4. DO NOT redraw, retype, reinterpret, resize, relocate, blur, distort, or modify ANY element on the product. "
                    "5. Even if the text is small or distant, it must remain SHARP and IDENTICAL to result. "
                    "6. NEVER TRANSLATE text. NEVER CORRECT spelling (even if 'IAMGE' or other typos exist). "
                    "If an area is plain/blank, keep it plain/blank. "
